...
def _walk(node, source: str, filepath: str, chunks: list, depth: int):
  ...
  """
  node_kind = node.kind()
  if node_kind in BLOCK_NODE_TYPES:
      name = _extract_name(node, source)
      content = source[node.start_byte():node.end_byte()]
      chunk_type = "class" if "class" in node_kind else "function"
      ...
      logger.debug(
          f"  Found {chunk_type} '{name}' (lines {start_position.row+1}-{end_position.row+1})"
      )
...
def _extract_name(node, source: str) -> str:
  ...
  for i in range(node.child_count()):
      ...
      if child_kind in ("identifier", "name", "property_identifier"):
          return source[child.start_byte():child.end_byte()]
  return node.kind()  # fallback to node kind if no name found
